...
export default function Proveedores({ BASE_URL }) {

    const { changeModalProveedor, modalProveedor, noAcces, usuario } = useUsuario();
    const [busqueda, setBusqueda] = useState('')

    const fetcher = () => axios(`${BASE_URL}/proveedor/listar-provedores/${usuario.token}`).then(datos => datos.data)
    const { data, error, isLoading } = useSWR(`${BASE_URL}/proveedor/listar-provedores/${usuario.token}`, fetcher, { refreshInterval: 100 })

    const proveedorFiltrado = busqueda === ''
        ? data
        : data.filter(proveedor => proveedor.nombre.toLowerCase().includes(busqueda.toLowerCase()))
                        ...
                        data && data.length ?
                            <div className='overflow-auto'>
                                ...
                                >
                                    ...
                                    <tbody>
                                        {
                                            proveedorFiltrado.map(proveedor =>
                                                <Proveedor
                                                    key={proveedor._id}
                                                    proveedor={proveedor}
                                                    BASE_URL={BASE_URL}
                            ...
                            :
                            <p className='text-center text-xl mt-10 uppercase'>Sin Proveedores</p>
                    }
